...
public class Process {
	...
	// rtlist(<dest,tag>,recv)
	public Hashtable<Pair<Integer, Integer>, LinkedList<Recv>> rtlist;

	public Hashtable<Integer, LinkedList<Send>> slist;
	// stlist(<dest,tag>,recv)
	public Hashtable<Pair<Integer, Integer>, LinkedList<Send>> stlist;
	...
	public void generateVE() {
		...
		rtlist = new Hashtable<Pair<Integer, Integer>, LinkedList<Recv>>();
		slist = new Hashtable<Integer, LinkedList<Send>>();
		stlist = new Hashtable<Pair<Integer, Integer>, LinkedList<Send>>();
		...
		for (Operation op : ops) {
			if (op instanceof Recv) {
				...
				if (!rtlist.containsKey(new Pair(r.dest, r.tag))) {
					rtlist.put(new Pair(r.dest, r.tag), new LinkedList<Recv>());
				}
				rtlist.get(new Pair(r.dest, r.tag)).add(r);

				if (lastr != null) {
					// we abandon the hb relation for a wildcard receive and a
					// following deterministic receive
					// because we do not consider this receive as a vertex in
					// graph
					// ***********************
				...
				// TODO: will r be added to vertices if it is a non-blocking
				// receive;
				// but the edge of match relation is still added to edges
				// what if a wait witnesses multiple receives?
				// how to add those match relations to the edges and only add
				// one wait to the vertices?
				vertices.add(r);
				// only assign a receive to be last one when it is a blocking
				// receive
				// or when its nw is witnessed when op is a wait
				// assume a program has only blocking receives or only
			...
			if (op instanceof Wait) {
				...
				// then add HB relations for the receives that this wait
				// witnesses
				if (wait.forR) {
			...
			if (op instanceof Send) {
				...
				if (!stlist.containsKey(new Pair(s.dest, s.tag))) {
					stlist.put(new Pair(s.dest, s.tag), new LinkedList<Send>());
				}
				stlist.get(new Pair(s.dest, s.tag)).add(s);
	...
	public void NextBlockPoint() {
		...
		while (indicator < ops.size()) {
			...
			if (ops.get(indicator) instanceof Recv)// right now, only receives
													// are considered as block
													// points
			{
	...
	public void ToPoint(Operation op) {
		// LinkedList<Operation> visitedOPs = new LinkedList<Operation>();
		while (indicator < ops.size()) {
			if (ops.get(indicator).equals(op))// right now, only receives are
												// considered as block points
				break;
			// visitedOPs.add(ops.get(indicator));
			indicator++;
		}
	...
	public boolean hasTagRecv() {
		boolean result = false;
		for (Operation op : ops) {
			if (op instanceof Recv) {
				if (((Recv) op).tag != -1)// r has a fixed tag
					return true;
			}
		}
		return result;
